...
public:
	...
	{
		plx_gri_locus &as;
		double y0, x0, sx2, sy2, sxy;

		ml_grri(plx_gri_locus &as_) : as(as_), mmizer(-.5, 2.5, 0, .001) { }

		// finds r-i for which the likelihood is maximum
		double operator()(float ri, float gr, float sg, float sr, float si)
		{
			y0 = ri; x0 = gr; 
		...
		double fn_to_minimize(double y)
		{
			...
			double x = as.gr(y);
			return peyton::sqr(x-x0)/sx2
			     - 2*sxy*(x-x0)*(y-y0)/(sx2*sy2)
			     + peyton::sqr(y-y0)/sy2;
		}
	} mlri;
