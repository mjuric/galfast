...
../../debug/src/simulate.x foot foot.rect.conf foot.xgpc.txt > output.log 2>&1
../../debug/src/simulate.x pdf pdf.conf foot.xgpc.txt model.BahcallSoneira.conf sky.pdf.bin >> output.log 2>&1
../../debug/src/simulate.x catalog catalog.conf sky.pdf.bin - 2>>output.log | awk '{if (count++%1000==0) print $0;}' > sky.cat.txt 
